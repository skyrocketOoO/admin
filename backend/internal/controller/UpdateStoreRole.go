package Middleware

import (
	"gorm.io/gorm"
	"taihe.asia/TeaSwap/api/V0"
	"taihe.asia/TeaSwap/internal/Common/Bytes"
	"taihe.asia/TeaSwap/internal/Common/Error"
	"taihe.asia/TeaSwap/internal/Common/PrepareBeforeAuth"
	"taihe.asia/TeaSwap/internal/Common/Struct"
	"taihe.asia/TeaSwap/internal/Common/Type"
	"taihe.asia/TeaSwap/internal/Connect"
	"taihe.asia/TeaSwap/internal/model/entity/Core/StoreRole"
)

func (it *Handler) UpdateStoreRole_Gob(
	context Type.Context,
	req *Connect.InnerRequest,
) (resp *Connect.InnerResponse, err error) {
	reqData, err := Bytes.ProtoDecode[V0.UpdateStoreRoleRequest](req.Msg.Data)
	if nil != err {
		err = Error.Throw(err)
		return
	}

	dbSet, operAccount, operRole, err := PrepareBeforeAuth.StoreGet(context)
	if nil != err {
		err = Error.Throw(err)
		return
	}
	if !operRole.Page.Role.Update {
		err = Error.New("not permitted")
		return
	}

	tgRole := StoreRole.StoreRole{}
	if err = dbSet.Core.Take(&tgRole, "ID = ?", reqData.GetID()).Error; err != nil {
		err = Error.Throw(err)
		return
	}
	if tgRole.Name == StoreRole.MinimumAuthorityName ||
		tgRole.Name == StoreRole.WebMasterName {
		err = Error.New("cannot update default role setting")
		return
	}

	if err = dbSet.Core.Transaction(func(tx *gorm.DB) (err error) {
		if reqData.GetName() != tgRole.Name {
			res := dbSet.Core.
				Where("StoreID = ?", operAccount.EntryID).
				Where("Name = ?", reqData.GetName()).
				Take(&StoreRole.StoreRole{})
			if res.Error != nil {
				if res.Error != gorm.ErrRecordNotFound {
					err = Error.Throw(res.Error)
					return
				}
			}
			if res.RowsAffected > 0 {
				err = Error.New("role name already exists")
				return
			}
		}

		tgRole.Name = reqData.GetName()
		if err = Struct.Scan(reqData.GetPage(), &tgRole.Page); err != nil {
			err = Error.Throw(err)
			return
		}

		if err = dbSet.Core.Save(&tgRole).Error; err != nil {
			err = Error.Throw(err)
			return
		}
		return nil
	}); err != nil {
		return
	}

	resp, err = Connect.NewProtoResponse(&V0.UpdateStoreRoleResponse{})
	if nil != err {
		err = Error.Throw(err)
		return
	}
	return
}
